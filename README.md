...
```mermaid
flowchart TD
    A[Setup batch file] --> A1[Configure startup and sync settings]

    A1 --> B[Setup parameter files]
    B --> B1[Configure streams and recording options]
    ...
    D --> E[Launch using batch file]
    D --> F[Launch using Watchdog]
